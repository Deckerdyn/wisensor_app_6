...
class _WeatherPageState extends State<WeatherPage> {
  ...
  Future<void> _fetchAlerts() async {
    ...
    http.Response response = await http.get(
      Uri.parse(
          "http://201.220.112.247:1880/wisensor/api/centros/alertas?ide=${widget.ide}&idu=${widget.idu}&idc=${widget.idc}"),
      headers: headers,
    );
  ...
  Future<void> _fetchWeather() async {
    ...
    for (int index = 0; index < _alerts.length; index++) {
      ...
      dynamic hdi =
          weather["heading_inicial"]; // Usar dynamic en lugar de double

      String key = "${emp}_${cce}_${cli}_${nr}_${dref}_${lat}_${lng}_${hdi}";

      http.Response response = await http.get(
        Uri.parse(
            "http://201.220.112.247:1880/wisensor/api/centros/alertas/clima?cli=$cli&emp=$emp&cce=$cce&nr=$nr&dref=$dref&lat=$lat&lng=$lng&hdi=$hdi"),
        headers: headers,
      );
                      ...
                      Expanded(
                        child: ListView.builder(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            _alerts.sort((a, b) {
                              DateTime dateA =
                                  DateTime.parse(parseDate(a["fecha"]));
                              DateTime dateB =
                                  DateTime.parse(parseDate(b["fecha"]));
                              return dateB.compareTo(
                                  dateA); // Reversed order for descending
                            });
